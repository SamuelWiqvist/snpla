...
for i in range(num_rounds):
    ...
    if hp_tuning == 0:
        ...
        np.savetxt('mv_gaussian/low_dim_w_summary_stats/hp_tuning/snl_posterior_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")
...
if hp_tuning == 0:
    ...
    with open('mv_gaussian/low_dim_w_summary_stats/hp_tuning/snl_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % hp_tuning)
        f.write('%.6f\n' % learning_rate)
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
