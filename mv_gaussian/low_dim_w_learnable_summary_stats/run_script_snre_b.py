...
for i in range(num_rounds):
    ...
    print(i)
    # TODO check if I can use the .net comp to run the sampling https://www.mackelab.org/sbi/reference/#sbi.inference.posteriors.ratio_based_posterior.RatioBasedPosterior
    # TODO however, that is most likely not possible...
    ...
    if hp_tuning == 0:
        ...
        np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/post_samples_snre_b_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")

    else:

        np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/hp_tuning/post_samples_snre_b_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")
...
if hp_tuning == 0:
    ...
    with open('mv_gaussian/low_dim_w_learnable_summary_stats/results/snre_b_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
        for i in range(num_rounds):
            f.write('%.4f\n' % kl_divs_trained[i])
    ...
    with open('mv_gaussian/low_dim_w_learnable_summary_stats/hp_tuning/snre_b_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % hp_tuning)
        f.write('%.6f\n' % learning_rate)
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
