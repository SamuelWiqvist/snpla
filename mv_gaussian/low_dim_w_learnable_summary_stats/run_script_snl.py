...
for i in range(num_rounds):
    ...
    if hp_tuning == 0:
        ...
        np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/snl_posterior_' + str(i + 1) + "_" + id_job + '.csv',
...
if hp_tuning == 0:
    ...
    with open('mv_gaussian/low_dim_w_learnable_summary_stats/results/snl_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
        for i in range(num_rounds):
            f.write('%.4f\n' % kl_divs_trained[i])
    ...
    with open('mv_gaussian/low_dim_w_learnable_summary_stats/hp_tuning/snl_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % hp_tuning)
        f.write('%.6f\n' % learning_rate)
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
...
if hp_tuning == 0:
    ...
    # save data
    np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_snl_' + id_job + '.csv',
               x_res_trained.detach().numpy(), delimiter=",")

    np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_prior_snl_' + id_job + '.csv',
               x_res_trained_prior.detach().numpy(), delimiter=",")

    np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_post_snl_' + id_job + '.csv',
