...
for i in range(nbr_rounds):
    ...
    np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/post_samples_spa_flow_' + str(i + 1) + "_" + id_job + '.csv',
...
with open('mv_gaussian/low_dim_w_learnable_summary_stats/results/spa_flow_' + id_job + '.txt', 'w') as f:
    f.write('%.4f\n' % run_time)
    f.write('%.4f\n' % run_time_inference)
    for i in range(nbr_rounds):
        f.write('%.4f\n' % kl_divs_trained[i])

np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_spa_flow_' + id_job + '.csv',
           x_res_untrained_obs.detach().numpy(), delimiter=",")

np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_prior_spa_flow_' + id_job + '.csv',
           x_untrained.detach().numpy(), delimiter=",")

np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/data_recon_post_spa_flow_' + id_job + '.csv',
