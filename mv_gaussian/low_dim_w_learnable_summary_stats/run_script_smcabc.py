...
np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(1) + "_" + id_job + '.csv',
...
np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(2) + "_" + id_job + '.csv',
           posterior_sample.detach().numpy(), delimiter=",")

posterior_sample = posterior_7dot5k.sample((1000,))
kl_div_7dot5k = conj_model.kl_div(analytical_posterior, posterior_sample)

np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(3) + "_" + id_job + '.csv',
           posterior_sample.detach().numpy(), delimiter=",")
...
np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(4) + "_" + id_job + '.csv',
           posterior_sample.detach().numpy(), delimiter=",")

posterior_sample = posterior_100k.sample((1000,))
kl_div_100k = conj_model.kl_div(analytical_posterior, posterior_sample)

np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(100) + "_" + id_job + '.csv',
           posterior_sample.detach().numpy(), delimiter=",")
...
np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/abcsmc_posterior_' + str(1000) + "_" + id_job + '.csv',
           posterior_sample.detach().numpy(), delimiter=",")

# save results
with open('mv_gaussian/low_dim_w_learnable_summary_stats/results/abcsmc_' + id_job + '.txt', 'w') as f:
    f.write('%.4f\n' % run_time)
    f.write('%.4f\n' % run_time_inference)
    f.write('%.4f\n' % kl_div_2dot5k)
    f.write('%.4f\n' % kl_div_5k)
