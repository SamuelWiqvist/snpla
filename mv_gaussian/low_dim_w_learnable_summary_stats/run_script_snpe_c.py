...
for i in range(num_rounds):
    ...
    if hp_tuning == 0:
        ...
        np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/data/post_samples_snpec_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")

    else:

        np.savetxt('mv_gaussian/low_dim_w_learnable_summary_stats/hp_tuning/post_samples_snpec_' + str(i + 1) + "_" + id_job + '.csv',
                       posterior_sample.detach().numpy(), delimiter=",")
...
if hp_tuning == 0:
    ...
    with open('mv_gaussian/low_dim_w_learnable_summary_stats/hp_tuning/snpec_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % hp_tuning)
        f.write('%.6f\n' % learning_rate)
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
