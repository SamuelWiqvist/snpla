...
for i in range(nbr_rounds):
    ...
    if hp_tuning == 0 and lambda_val > 0:
        ...
        np.savetxt('mv_gaussian/low_dim_w_five_obs/lambda_val/post_samples_snpla_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")

    elif hp_tuning == 0:

        np.savetxt('mv_gaussian/low_dim_w_five_obs/data/post_samples_snpla_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")

    else:

        np.savetxt('mv_gaussian/low_dim_w_five_obs/hp_tuning/post_samples_snpla_' + str(i + 1) + "_" + id_job + '.csv',
                   posterior_sample.detach().numpy(), delimiter=",")
...
if hp_tuning == 0 and lambda_val > 0:
    ...
    with open('mv_gaussian/low_dim_w_five_obs/lambda_val/snpla_' + id_job + '.txt', 'w') as f:
    ...
    with open('mv_gaussian/low_dim_w_five_obs/results/snpla_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % run_time)
        f.write('%.4f\n' % run_time_inference)
        for i in range(nbr_rounds):
            f.write('%.4f\n' % kl_divs_trained[i])
    ...
    with open('mv_gaussian/low_dim_w_five_obs/hp_tuning/snpla_' + id_job + '.txt', 'w') as f:
        f.write('%.4f\n' % hp_tuning)
        for h in hyper_params:
            f.write('%.6f\n' % h)
        f.write('%.4f\n' % run_time)
...
if hp_tuning == 0:
    ...
    np.savetxt('mv_gaussian/low_dim_w_five_obs/data/data_recon_snpla_' + id_job + '.csv',
               x_res_untrained_obs.detach().numpy(), delimiter=",")

    np.savetxt('mv_gaussian/low_dim_w_five_obs/data/data_recon_prior_snpla_' + id_job + '.csv',
               x_untrained.detach().numpy(), delimiter=",")

    np.savetxt('mv_gaussian/low_dim_w_five_obs/data/data_recon_post_snpla_' + id_job + '.csv',
